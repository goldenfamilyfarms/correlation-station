...
class StartClass:
    '''This script passes params to meta_main.py to check for
    connectivity related errors

    note to self:
        may add key to flip on/off the refresh of the log gathering tool api so when testing it wont be
    ...
    param_dictionary = {
        ...
        "get_ip_and_fqdn_from_nf": True,
        # refresh_logs - Choose to run api call to run log gather
        "refresh_logs": True,
        # webex_room_id - Add an id for the room and add META bot to that room
        # room name = connectivity_report_room_id
        "webex_room_id": "Y2lzY29zcGFyazovL3VzL1JPT00vYjQxZDE3MjAtNDVkNS0xMWVlLTk3NDQtY2ZjNzg1YWQ0NjZh",
