...
class Dataframe(ProductError):
    ...
    def check_if_error_is_categorized(self, error_item):
        ...
        '''
        for row in self.cur.execute("SELECT error_code, regex_pattern FROM defects ORDER BY error_code"):
            error_code = row[0]
            regex_pattern = row[1]
            check = re.search(regex_pattern, error_item)
            if check:
                return error_code
    ...
    def categorize_and_count_errors_found(self, dataframe):
        ...
        # IDEA to keep info strait #### remove when done
        dataframe = product daily spreadsheet
        self.categorized_error_dataframe = compliance_automation_error_codes.xlsx
        '''
        categorized_error_count = {}
        for row in self.cur.execute("SELECT error_code, regex_pattern FROM defects ORDER BY error_code"):
            error_code = row[0]
            regex_pattern = row[1]
            test = dataframe.loc[dataframe['Error'].str.contains(regex_pattern), :]
            categorized_error_count[regex_pattern] = [test.shape[0], error_code]
