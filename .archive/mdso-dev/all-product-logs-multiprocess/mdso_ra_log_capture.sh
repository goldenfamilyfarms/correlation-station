...
for host in "${hosts[@]}"; do for file in $(ssh msmith@$host " grep -rl --exclude=*.{gz.*,gz,log.*} bpprov.$provider_resource_id /opt/ciena/bp2/$ra_vender_dir*/log"); do scp -r msmith@$host:$file "$log_dir_path"/"$date_format"/"$sub_dir_name"/"$fqdn_dir_name"; done; done
