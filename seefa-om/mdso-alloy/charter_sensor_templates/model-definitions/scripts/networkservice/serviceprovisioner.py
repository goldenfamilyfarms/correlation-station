...
class Activate(CompleteAndTerminatePlan, OTelMixin):
    ...
    def process(self):
        ...
        with self.create_root_span():
            ...
            # Record span event for provisioning start
            ...
            # Add topology attributes from circuit details
            if getattr(self, '_otel_initialized', False):
                self.add_topology_attributes_to_span(
                    service_type=service_type,
                    topology_node_count=len(circuit_details.get("properties", {}).get("topology", [{}])[0].get("data", {}).get("node", [])) if circuit_details.get("properties", {}).get("topology") else None
                )
            ...
            if self.context == "PE" and self.PE_SPECIFIC_RESOURCE_TYPES.get(service_type):
                # Use granular span for PE provisioning
                with self.create_network_function_span_context(
                    ...
                    if getattr(self, '_otel_initialized', False):
                        self.add_network_function_attributes_to_span(
                            span=pe_span,
                            device_role="PE",
                            vendor=None  # Will be extracted from topology
                        )
                        self.record_span_event_from_instance(
                            "pe.provisioning.started",
                            {"service_type": service_type}
                        )
                    
                    # this calls service fia provisioner
                    with self.timed_operation("pe.provision", {"service_type": service_type, "context": "PE"}):
        ...
        self.logger.debug("MEF Service: " + str(mef_service))
            ...
            with self.timed_operation("mef.service.operation", {"service_type": service_type, "context": self.context}):
                if self.is_mef_service_created(mef_service):
                    self.logger.info("updating mef service")
                    if getattr(self, '_otel_initialized', False):
                        self.record_span_event_from_instance("mef.service.update.started")
                    self.update_mef_service(mef_service)
                    if getattr(self, '_otel_initialized', False):
                        self.record_span_event_from_instance("mef.service.update.completed")
                else:
                    self.logger.info("creating mef service")
                    if getattr(self, '_otel_initialized', False):
                        self.record_span_event_from_instance("mef.service.create.started")
                    self.create_mef_service(mef_service, service_resource_type, network_service["id"])
                    if getattr(self, '_otel_initialized', False):
                        self.record_span_event_from_instance("mef.service.create.completed")

            # UPDATE CPE STATES IN NETWORKSERVICE
            self.logger.debug("CPE Update list: " + str(cpe_update_list))
            for cpe in cpe_update_list:
                with self.timed_operation("cpe.state.update", {"cpe": cpe}):
                    update_site_state.update_site_state_by_network_service(network_service, cpe, "ACTIVATING-INITIATED")

            with self.timed_operation("circuit_details.update", {"device_count": len(original_device_update_list)}):
            ...
            if getattr(self, '_otel_initialized', False):
                ...
                if hasattr(self, 'metrics'):
                    self.metrics.record_provisioning(
                        ...
                        device_count=len(original_device_update_list)
                    )
                self.record_span_event_from_instance(
