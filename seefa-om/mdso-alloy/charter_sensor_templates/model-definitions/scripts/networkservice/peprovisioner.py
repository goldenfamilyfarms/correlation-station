...
_import_logger.info("peprovisioner.py: Attempting to import OTelMixin from scripts.otel.otel_mixin")
try:
    from scripts.otel.otel_mixin import OTelMixin
    _import_logger.info("peprovisioner.py: Successfully imported OTelMixin")
except ImportError as e:
    _import_logger.error(f"peprovisioner.py: Failed to import OTelMixin - Error: {e}")
    raise
import re
import json


class PeProvisioner(CommonPlan, OTelMixin):
    """
    ...
    def create_pe_tpe(self, properties):
        ...
        if not hasattr(self, '_otel_initialized'):
            self.__init_otel__()
        
        # Use granular span for PE TPE creation
        pe_device_id = properties.get("pe_device_rid", "unknown")
        with self.create_network_function_span_context(
            pe_device_id,
            operation="create_tpe"
        ) as tpe_span:
            if getattr(self, '_otel_initialized', False):
                self.record_span_event_from_instance(
                    "pe.tpe.creation.started",
                    {
                        "pe_device_rid": pe_device_id,
                        "port_id": properties.get("port_id"),
                        "interface_label": properties.get("interface_userLabel")
                    }
                )
            
            # Getting required domain
            with self.timed_operation("pe.tpe.domain.get"):
            ...
            if getattr(self, '_otel_initialized', False):
                self.record_span_event_from_instance(
                    "pe.tpe.creation.completed",
                    {"tpe_resource_id": pe_tpe_res.resource_id}
                )
