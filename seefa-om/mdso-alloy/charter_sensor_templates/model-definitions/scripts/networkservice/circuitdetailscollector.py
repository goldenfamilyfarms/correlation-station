...
class Activate(CommonPlan, OTelMixin):
    ...
    def process(self):
        ...
        with self.create_root_span():
            ...
            # Use topology span context for Beorn operations
            with self.create_topology_span_context(self.circuit_id, "fetch") as topology_span:
                topology = self.get_topology_from_sense()
                
                # Validate and instrument Beorn response
                is_valid = self.validate_and_instrument_beorn_response(topology)
                if not is_valid:
                    self.otel_log("Beorn response validation failed", level="warning")
                
                # Extract and set topology context (vendor, FQDN, etc.)
                self.extract_and_set_topology_context(topology)
                
                # Add topology attributes to span
                service_type = topology.get("serviceType") if isinstance(topology, dict) else None
                if service_type:
                    self.add_topology_attributes_to_span(
                        span=topology_span,
                        service_type=service_type,
                        topology_node_count=len(topology.get("topology", [{}])[0].get("data", {}).get("node", [])) if isinstance(topology, dict) and topology.get("topology") else None,
    ...
    def get_topology_from_sense(self):
        ...
        if self.bpo_constants is None:
            ...
            # Use OTel error handler if available
        ...
        # Record span event for topology fetch
        if getattr(self, '_otel_initialized', False):
            self.record_span_event_from_instance(
                "beorn.topology.fetch.started",
                {"url": url, "use_alternate_url": self.use_alternate_url}
            )
        ...
        # Record span event for topology fetch completion
        if getattr(self, '_otel_initialized', False):
            self.record_span_event_from_instance(
                "beorn.topology.fetch.completed",
                {"topology_size": len(str(topology))}
            )
    ...
    def create_circuit_details(self, parent_id, circuit_details):
        ...
        try:
            ...
            # Use OTel error handler if available
            if getattr(self, '_otel_initialized', False):
                self.otel_error_handler(str(ex), exception=ex)
