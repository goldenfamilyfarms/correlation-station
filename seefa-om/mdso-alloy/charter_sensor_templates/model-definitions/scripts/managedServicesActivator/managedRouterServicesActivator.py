...
_import_logger.info("managedRouterServicesActivator.py: Attempting to import OTelMixin from scripts.otel.otel_mixin")
try:
    from scripts.otel.otel_mixin import OTelMixin
    _import_logger.info("managedRouterServicesActivator.py: Successfully imported OTelMixin")
except ImportError as e:
    _import_logger.error(f"managedRouterServicesActivator.py: Failed to import OTelMixin - Error: {e}")
    raise
...
class Activate(CommonPlan, CliCutthrough, OTelMixin):
    ...
    def process(self):
        ...
        with self.create_root_span():
            ...
            # Add custom attributes for MRS activation
            if getattr(self, '_otel_initialized', False):
                span = trace.get_current_span()
                if span and span.is_recording():
                    span.set_attribute("mrs.vendor", self.vendor.upper())
                    span.set_attribute("mrs.model", self.model)
                    span.set_attribute("mrs.fqdn", self.fqdn)
                    span.set_attribute("mrs.ip_address", self.ipAddress)
                    span.set_attribute("mrs.operation_type", self.operationType)
                    if self.firmwareVersion:
                        span.set_attribute("mrs.firmware_version", self.firmwareVersion)
                
                self.record_span_event_from_instance(
                    "mrs.activation.started",
                    {
                        "vendor": self.vendor,
                        "model": self.model,
                        "fqdn": self.fqdn,
        ...
        msg = "Onboarding Device"
            ...
            # Use network function span for device onboarding
            with self.create_network_function_span_context(
                ...
                if getattr(self, '_otel_initialized', False):
                    ...
                    self.record_span_event_from_instance("mrs.device.onboard.started")
                
                try:
                    onboard_start = time.time()
                    nf = self.onboard_network_function(fqdn)
                    onboard_duration = (time.time() - onboard_start) * 1000
                    ...
                    if getattr(self, '_otel_initialized', False):
                        if nf and nf.get("id"):
                            self.add_network_function_attributes_to_span(
                                span=nf_span,
                                provider_resource_id=nf["id"],
                                communication_state="onboarded"
                            )
                        if hasattr(self, 'metrics'):
                            self.metrics.record_device_onboard(
                                vendor=self.vendor.upper(),
                                duration_ms=onboard_duration,
                                success=True
                            )
                        self.record_span_event_from_instance("mrs.device.onboard.completed")
                except MRSA_Exception as err:
                    err_msg = "Failed: {} - Reason: {}".format(msg, err)
                    if getattr(self, '_otel_initialized', False):
