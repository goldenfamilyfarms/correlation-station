...
_import_logger.info("deviceonboarder.py: Attempting to import OTelMixin from scripts.otel.otel_mixin")
try:
    from scripts.otel.otel_mixin import OTelMixin
    _import_logger.info("deviceonboarder.py: Successfully imported OTelMixin")
except ImportError as e:
    _import_logger.error(f"deviceonboarder.py: Failed to import OTelMixin - Error: {e}")
    raise
    ...
    def process(self):
        ...
        with self.create_root_span():
            ...
            if not vendor_resource_types:
                error_msg = f"Device vendor '{device_vendor}' is not defined in common_plan.py"
                if getattr(self, '_otel_initialized', False):
                    self.otel_error_handler(error_msg)
                raise Exception(error_msg)
            device_resource_type = vendor_resource_types["DEVICE_TYPE"]

            # Use network function span for device operations
            # Extract FQDN from device_name or device_ip
            fqdn = device_name if device_name else device_ip
            tid = None  # TID may not be available at this stage
            
            with self.create_network_function_span_context(
                tid or device_ip,  # Use IP as identifier if TID not available
                fqdn=fqdn,
                operation="onboard"
            ) as nf_span:
                ...
                self.add_network_function_attributes_to_span(
                    ...
                    device_role="CPE"  # Default, may be updated later
                )
                ...
                if len(existing_devices["items"]) > 0:
                    ...
                    if getattr(self, '_otel_initialized', False):
                        self.record_span_event_from_instance(
                            "device.onboarding.skipped",
                            {"reason": "device_already_exists", "device_ip": device_ip}
                        )
                    return
        ...
        if device_ip.find(".") > 0:
            ...
            if devs:
                ...
                if domain is None:
                    error_msg = "Unable to find an available domain, please check session profiles."
                    if getattr(self, '_otel_initialized', False):
                        self.otel_error_handler(error_msg)
                    raise Exception(error_msg)
                ...
                if getattr(self, '_otel_initialized', False):
                    self.record_span_event_from_instance(
                        "device.onboarding.creating",
                        {"device_name": device_name, "domain_id": domain_id}
                    )
                ...
                try:
                    ...
                    # Update network function attributes with new device info
                    if getattr(self, '_otel_initialized', False) and new_device:
                        self.add_network_function_attributes_to_span(
                            span=nf_span,
                            provider_resource_id=new_device.get("id"),
                            communication_state="onboarding"
                        )
                        self.record_span_event_from_instance(
                            "device.onboarding.created",
                            {"device_id": new_device.get("id"), "device_name": device_name}
                        )
                    ...
                    if getattr(self, '_otel_initialized', False):
                        self.otel_error_handler(msg, exception=e)
                    self.exit_error(msg)
                ...
                if operation != "MANAGED_SERVICES_ACTIVATION":
                    ...
                    try:
                        ...
                        if getattr(self, '_otel_initialized', False):
                            self.add_network_function_attributes_to_span(
                                span=nf_span,
                                communication_state="synced"
                            )
                            self.record_span_event_from_instance(
                                "device.onboarding.completed",
                                {"device_id": new_device["id"], "device_name": device_name}
                            )
                    except Exception:
                        msg = (
