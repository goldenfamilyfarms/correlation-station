...
resource = Resource.create({"service.name": "sense-compliance"})
provider = TracerProvider(resource=resource)
provider.add_span_processor(BatchSpanProcessor(OTLPSpanExporter(endpoint=OTEL_ENDPOINT, insecure=True)))
trace.set_tracer_provider(provider)
tracer = trace.get_tracer(__name__)

DATA_FILE = os.path.join(os.path.dirname(__file__), '..', 'mock', 'data', 'fiber_internet.json')
